...
void slim::IR::dumpIR()
{
    ...
    std::string op_type[] = {"+", "-", "*", "/", "%", "<<", ">>>", ">>", "&", "|", "^"};
    std::vector<std::set<std::string>> genSet(total_instructions);
    std::vector<std::set<std::string>> killSet(total_instructions);
    ...
    for (auto &entry : this->func_bb_to_inst_id)
    {
        ...
        for (long long instruction_id : entry.second)
        {
            ...
            if (ops > 0)
            {
                ...
                {
                    SLIMOperand *slimOperand = instruction->getOperand(i).first;

                    // Check if the operand is not an integer
                    if (!slimOperand->isInteger())
                    {
                        std::string operandName = slimOperand->getName().str(); // Convert llvm::StringRef to std::string
                        // llvm::outs() << operandName << " ";
                        allVariables.insert(operandName);
                    }
                }
                // llvm::outs() << "\n";
            }
            ...
            llvm::outs() << "Type : " << ins_type[instruction->getInstructionType()] << " instruction"
                         << "\n";
        }
    ...
    for (auto &entry : this->func_bb_to_inst_id)
    {
        ...
        for (long long instruction_id : entry.second)
        {
            ...
            // llvm::outs() << "INSTRUCTION TYPE : " << ins_type[instruction->getInstructionType()] << "\n";
            ...
            if (ops > 0)
            {
                ...
                for (unsigned i = 0; i < ops; i++)
                {
                    ...
                    if (!slimOperand->isInteger())
                    {
                        std::string operandName = slimOperand->getName().str(); // Convert llvm::StringRef to std::string
                        // llvm::outs() << operandName << " ";
                        killSet[instruction_id].insert(operandName);
                    }
                }
                // llvm::outs() << "\n";
            }
            ...
            if (instruction->getInstructionType() == InstructionType::BINARY_OPERATION ||
                instruction->getInstructionType() == InstructionType::LOAD ||
                instruction->getInstructionType() == InstructionType::STORE || 
                instruction->getInstructionType() == InstructionType::COMPARE ||
                instruction->getInstructionType() == InstructionType::ALLOCA||
                instruction->getInstructionType() == InstructionType::PHI
                )
            {
                std::string lhsName = instruction->getLHS().first->getName().str(); // Convert llvm::StringRef to std::string
                
                genSet[instruction_id].insert(lhsName);
            }
    ...
    for (auto &entry : this->func_bb_to_inst_id)
    {
        ...
        if (func_visited.find(func) != func_visited.end())
        {
            // Function has already been visited, skip it
            continue;
        }
        ...
        // Creating DeadIN DeadOUT for each BASICBLOCK
        std::vector<std::set<std::string>> BB_DeadIN(total_basic_blocks);
        std::vector<std::set<std::string>> BB_DeadOut(total_basic_blocks);

        // Creating DeadIn DeadOut for each instruction in a basic block
        std::vector<std::set<std::string>> DeadIn(total_instructions);
        std::vector<std::set<std::string>> DeadOut(total_instructions);

        // iterate till it converge

        int iteration = 0;
        bool changed = true;
        while (changed)
        {
            std::cout << "Ieration #" << iteration << "\n\n";
            changed = false;

            // Iterate over basic blocks in reverse post-order
            ...
            for (; I >= RPOT.begin(); --I)
            {
                ...
                std::cout << "Visiting Basic Block: " << currentBlock->getName().str() << std::endl
                          << "\n";
                // Print the basic block name
                llvm::outs() << "Basic block " << curr_BlockId << ": " << currentBlock->getName() << "\n";
                ...
                // DeadOut OF BASICBLOCK

                if (iteration == 0)
                {
                    BB_DeadOut[curr_BlockId] = allVariables;
                    BB_DeadIN[curr_BlockId] = allVariables ;
                } 
               
                    for (auto succ = llvm::succ_begin(currentBlock); succ != llvm::succ_end(currentBlock); succ++)
                    {
                        // Get the successor basic block
                        llvm::BasicBlock *successorBlock = *succ;

                        // Retrieve the ID of the successor basic block
                        int successorID = this->getBasicBlockId(successorBlock);

                        // If it's the first successor, initialize BB_DeadOut with its In set
                        if (succ == llvm::succ_begin(currentBlock))
                        {
                            BB_DeadOut[curr_BlockId] = BB_DeadIN[successorID];
                        }
                        else
                        {
                            // Perform an intersection operation with the In set of the current successor
                            BB_DeadOut[curr_BlockId] = intersectSets(BB_DeadOut[curr_BlockId], BB_DeadIN[successorID]);
                        }
                    }
                ...
                std::set<std::string> newDeadOut = BB_DeadOut[curr_BlockId];
                std::set<std::string> newDeadIn;

                for (long long instruction_id : reverseInstList)
                {
                    BaseInstruction *instruction = inst_id_to_object[instruction_id];
                    ...
                    if (instruction)
                    {
                        ...
                        DeadOut[instruction_id] = newDeadOut;

                        // DeadIn =genset union (out-kill)
                        if (!genSet[instruction_id].empty())
                        {
                            DeadIn[instruction_id].insert(genSet[instruction_id].begin(), genSet[instruction_id].end());
                        }
                        if (!DeadOut[instruction_id].empty())
                        {
                            if (!killSet[instruction_id].empty())
                            {
                                for (const std::string &variable : killSet[instruction_id])
                                {
                                    DeadOut[instruction_id].erase(variable); // Remove killset from DeadOut
                                }
                                DeadIn[instruction_id].insert(DeadOut[instruction_id].begin(), DeadOut[instruction_id].end());
                            }
                            else
                            {
                        ...
                        // Printing GEN KILL DeadIn DeadOut of each INSTRUCTION
                        llvm::outs() << "Gen  :";
                        for (const auto &killItem : genSet[instruction_id])
                        {
                            llvm::outs() << killItem << " ";
                        }
                        llvm::outs() << "     "
                                     << "Kill   :";
                        for (const auto &genItem : killSet[instruction_id])
                        {
                            llvm::outs() << genItem << " ";
                        }
                        llvm::outs() << "     "
                                     << "Dead Out  :";
                        for (const auto &DeadOutItem : newDeadOut)
                        {
                            llvm::outs() << DeadOutItem << " ";
                        }
                        llvm::outs() << "      "
                                     << "Dead In  :";
                        for (const auto &DeadInItem : DeadIn[instruction_id])
                        {
                            llvm::outs() << DeadInItem << " ";
                        }

                        // copying in newDeadOut to pass to the next DeadOut;
                        newDeadOut = DeadIn[instruction_id];
                    }
                    else
                    {
                        // Handle the case where the instruction couldn't be found
                        // (e.g., by printing an error message)
                        std::cerr << "Error: Instruction with ID " << instruction_id << " not found." << std::endl;
                    }

                    llvm::outs() << "\n";
                }
                if (BB_DeadIN[curr_BlockId] != newDeadOut)
                {
                    changed = true;
                    BB_DeadIN[curr_BlockId] = newDeadOut;
                }
                // BB_DeadIN[curr_BlockId]=newDeadOut;

                llvm::outs() << "   BB Dead out : ";
                for (const auto &DeadOutItem : BB_DeadOut[curr_BlockId])
                {
                    llvm::outs() << DeadOutItem << " ";
                }
                llvm::outs() << "   BB Dead In : ";
                for (const auto &DeadInItem : BB_DeadIN[curr_BlockId])
                {
                    llvm::outs() << DeadInItem << " ";
                }
